---
title: "第十章-作业"
author: "陈建杰(2022190065)"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## 练习题

1. 读入数据`Credit.csv`。
```{r}
Credit =  read.csv("Credit.csv")
head(Credit)
```
数据包含如下变量：

- balance: average credit card debt for a number of individuals
- cards: number of credit cards
- education: years of education
- income (in thousands of dollars)
- limit (credit limit)
- rating (credit rating)
- Married (marital status)
- ethnicity (Caucasian, African American or Asian)
- gender
- student (student status)
- age


2. 以`Rating`为横轴，`Balance`为纵轴画散点图，横轴坐标名称为`Rating`，纵轴坐标名称为`Balance`。点的类型为`+`。

```{r fig.retina=5, fig.align='center', fig.height=5, fig.width=7}
plot(Credit$Rating, Credit$Balance, pch = '+', xlab = 'Rating', ylab = 'Balance', main = 'Scatter Plot of Rating vs Balance')
```

3. 画出`Balance`的直方图，设置参数`freq=F`。并且画出`Balance`的概率密度函数。注：可能需要用到函数`density()`。

```{r fig.retina=5, fig.align='center', fig.height=5, fig.width=7}
# 绘制直方图
hist(Credit$Balance, freq = F, main = "Histogram of Balance", xlab = "Balance")

# 计算 Balance 的概率密度函数
density_data <- density(Credit$Balance)

# 绘制概率密度函数
lines(density_data, col = "red")
```


4. 画出学生的`Balance`的箱型图和非学生的`Balance`的箱型图。两个箱型图分别用两种不同的颜色表示。

```{r fig.retina=5, fig.align='center', fig.height=5, fig.width=7}

# 绘制箱型图
boxplot(Balance ~ Student, data = Credit, col = c("blue", "green"))
title("Boxplots of Balance for Students and Non-students")

```












